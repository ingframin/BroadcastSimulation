...
def cleanup(line):
    return line.replace(';',',').replace('(','').replace("'","").replace(')','').replace('b','').replace('\\t',',').replace('\\r\\n','').replace('"','').strip()
...
##        plt.hist(h,bins=30,density=True,histtype='step')
    xy1 = plt.hist(h_xy[0],density=True,histtype='bar',align='left',rwidth=0.8)
    xy2 = plt.hist(h_xy[1],density=True,histtype='bar',align='left',rwidth=0.8)
    plt.close()
    plt.plot(range(0,10),xy2[0],'*g',markersize = 25)
    plt.plot(range(0,10),xy1[0],'*r',markersize = 25)
    plt.axis([0,10,0,0.25])
    plt.xlabel('Throughput [msg/s]',fontsize=20)
    plt.ylabel('PDF',fontsize=20)
    plt.xticks(range(0,10),fontsize=20)
